FROM node:20-alpine
RUN npm install -g @modelcontextprotocol/server-proxy supergateway
...
EXPOSE 8080
ENTRYPOINT ["supergateway", "--port", "8080", "--stdio", "github-mcp-server stdio", "--outputTransport", "streamableHttp", "--allowOrigin", "*"]
